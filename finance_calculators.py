...
# Create calculator option for user input and transform into lowercase. Also create option to remove user error input for any spacing in wording.
...
if calc_type == "investment":
    ...
    # Simple interest total printed to user.(including formula display to user total simple interest calculation)
    ...
    # Compond interest total printed to user.(including formula and display to user total compound interest calculation)
...
# Condtional statement and input for bond option.(including formula and display to user bond month repayment calculation)
elif  calc_type == "bond":
    house_value = float(input("\n Please enter present value of house: "))     
    rate = int(input("Please enter your interest rate as a number: "))
...
# Calculations for monthly percentage
    ...
    total_repayment = round(interest_rate * house_value)/(1 -(1 + rate)**(-n_month))
    print(f"\n Your monthly bond repayment will be £{total_repayment}")
